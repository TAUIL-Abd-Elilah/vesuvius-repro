...
| **Whether a falling Spiral loss means a better fit** | [`spiral_quality/`](spiral_quality) | **negative**: loss -82.30%, but CT intervals are neutral and median pitch collapses 15.99 → 3.27 |
...
## Spiral quality: the loss improved and the fit collapsed

[`spiral_quality/`](spiral_quality) releases a registered paired evaluation of one bounded
PHerc0125 production fit. Optimization loss fell from 111.705 to 19.777, but all three paired CT
intervals spanned zero and median pitch fell from 15.990 to 3.268. Collapsed-spacing,
inflated-spacing, pitch-instability and radial-order alerts fired, so every authorization decision
is false.

The included offline verifier rehashes 800 stored CT profiles and recomputes the cluster-bootstrap
intervals, intrinsic deltas and decisions. This demonstrates that the gate catches self-fitting;
it does not claim the fit improved, generalize beyond the exact 896-slice slab, or satisfy the
stronger requirement that diagnostics enable a subsequent improvement.
